...
-   `ddist(x, p1, p2, ...)`는 $X$가 이산형일 경우 $P(X=x)$를, 연속형일 경우 $X$의 밀도를 나타낸다.

-   `pdist(q, p1, p2, ...)`는 $P(X \leq q)$를 나타낸다.

-   `qdist(p, p1, p2, ...)`는 $P(X \leq q) \geq p$가 되는 가장 작은 $q$를 나타낸다 (100$p$%-포인트).

-   `rdist(n, p1, p2, ...)`는 분포 유형 `dist`로부터 난수 벡터 `n`을 생성한다.

`x`, `p`, `q`는 모두 벡터 값이 될 수 있으며, 이 경우 출력도 벡터 값이다.

## Some of the discrete distributions

R에서 제공하는 몇 가지 이산 분포와 관련 입력 매개변수의 이름은 다음과 같다.
...
이항 분포를 따르는 $X$에 대해 `size=10`, `prob=0.5`일 때 $P(X \leq 5)$는 다음과 같다.
...
이는 주어진 확률 $p$에 대해, 누적확률이 $p$가 넘어서는 이항 확률변수 값들 중 가장 작은 값을 찾는다.
...
여러 parameter 값들에 따른 확률질량함수(probability mass function)들을 관찰해 보자.
...
-   `factor` 함수를 이용하면, `levels`을 명시하여 다음과 같이 나타낼 수 있다.
...
표본의 크기 $N$이 크면, 확률 변수의 상대 빈도는 확률 질량 함수(pmf)와 유사하다.
...
고장률이 0.01일 때, 무작위로 선택된 1000개의 물품 중 20개 미만의 고장이 발생할 확률은 무엇인가?

$X$가 $p=0.01$의 이항확률변수일 때, $\mathbb P(X < 20)$을 구하는 것과 같다.
...
$B_1, B_2, \ldots$는 $Bernoulli(p)$ 확률 변수가 독립적으로 이루어진 무한 수열이라고 하자.

그리고 $Y$를 $B_1 = \cdots = B_Y = 0$이고 $B_{Y+1} = 1$인 확률률변수라고 하자.
...
위에서 했던 것과 비슷하게, `rgeom`을 통해 생성된 기하확률변수의 샘플 히스토그램과 pmf를 비교하여 보자.
...
이번에는 `rgeom`이 아니라, 기하확률변수의 정의에 따라 베르누이 시행을 바탕으로 확률변수를 생성해 보자.
...
$Z$를 독립적이고 동일하게 분포된 $Bernoulli(p)$ 시행에서 $r$번째 성공이 나타나기 전까지의 실패 횟수라고 하자.

그렇다면 $Z$는 음이항 분포를 따르며, $Z \sim nbinomial(r, p)$로 표기한다.
...
다음 코드는 $r = 3$번째 성공이 나타날 때까지의 실패 횟수를 세며, 각 베르누이 시행의 성공 확률 $p=0.4$이다.
...
어느 제조업체는 각 배치에서 무작위로 100개의 제품을 선택하여 생산 품질을 검사한다.

불량품이 2개를 초과하면 생산을 중단한다.

-   불량 확률 : $p$

100개의 제품을 검사했을 때 (3개 이상의 불량품이 발견되어) 생산이 중단될 확률을 계산해 보자.
...
만약 $Z$가 $nbinomial(r, p)$를 따른다면, $P(Z \leq z) = 1 - P(X \leq r - 1)$이 성립하며, 여기서 $X$는 $binom(z + r, p)$를 따른다.
...
포아송 분포는 시간이나 공간에서 무작위로 발생하는 사건을 모델링하는 데 사용된다.
...
$X$가 매개변수 $\lambda$를 가지는 포아송 분포를 따른다고 하고, 이를 $X \sim \text{pois}(\lambda)$로 표기한다.
...
$n$이 크고 $p$가 작을 때, $binom(n, p)$는 $\mathrm{pois}(np)$로 근사된다.
